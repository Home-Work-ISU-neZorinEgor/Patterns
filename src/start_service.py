...
class StartService:
    ...
    def __create_transaction(self):
        ...
        nomenclature = self.__repository.data[DataRepository.nomenclature_id()][0]
        transaction_type = TransactionType.INBOUND
        quantity = 1
        range = self.__repository.data[DataRepository.range_id()][0]
        time = datetime.datetime.now(datetime.timezone.utc)
        transaction = StorehouseTransaction.create(storehouse, nomenclature, transaction_type, quantity, range, time)
        self.__repository.data[DataRepository.transaction_id()] = [transaction]
